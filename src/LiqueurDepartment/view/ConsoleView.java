...
public class ConsoleView {
    ...
    // 주류 ID 선택 (LiqueurStorage 사용)
    public int selectLiqueurId(Object obj, String promptMessage) {
        int liqueurId;
        boolean result;
        do {
            if (promptMessage == null) {
                promptMessage = "주류의 ID를 입력하세요: "; // 기본 메시지
            }
            System.out.print(promptMessage);
            liqueurId = readNumber();
            if (obj instanceof LiqueurStorage) {
                result = ((LiqueurStorage) obj).isValidLiqueur(liqueurId);
            } else if (obj instanceof Cart) {
                result = ((Cart) obj).isValidLiqueur(liqueurId);
            } else {
                result = false;
            }
            if (!result) {
                System.out.println("잘못된 주류의 ID입니다.");
            }
        } while (!result);
    ...
    // 주류 상세 정보 입력 받기
    public Liqueur inputLiqueurDetails(int id) {
        Scanner input = new Scanner(System.in);

        System.out.print("주류 ID: " + id + "\n");
