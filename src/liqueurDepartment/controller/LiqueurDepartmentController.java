...
import liqueurDepartment.model.*;
import liqueurDepartment.view.ConsoleView;
import liqueurDepartment.order.*;
import liqueurDepartment.user.customer.Customer;
import liqueurDepartment.user.admin.Admin;


public class LiqueurDepartmentController {
    private final ConsoleView view;
    private final LiqueurStorage mLiqueurStorage;
    ...
    String[] adminMenuList = { "0. 종료", "1. 주류 정보 보기", "2. 주류 추가", "3. 주류 삭제", "4. 주류 수정", "5. 모든 주문 보기", "6. 출고 처리",
            "7. 로그아웃" };
    ...
    private void searchByNameOrType() {
        ...
        if (keyword.length() >= 3) {
            ArrayList<Liqueur> results = mLiqueurStorage.searchLiqueurByNameOrType(keyword);
    ...
    private void order() {
        ...
        if (view.askConfirm("진짜 주문하려면 yes를 입력하세요 : ", "yes")) {
            ...
            orderDetails.append(String.format("주문 ID: %d, 고객: %s, 총 금액: %d원, 주소: %s, 전화번호: %s, 이메일: %s, ", orderId,
                    mCustomer.getName(), mCart.getTotalPrice(), mCustomer.getAddress(), mCustomer.getPhone(),
                    mCustomer.getEmail()));

            for (CartItem item : mCart.getItemList()) {
                orderDetails.append(String.format("주류: %s, 수량: %d, ", item.getLiqueur().getLiqueurName(), item.getQuantity()));
            }
